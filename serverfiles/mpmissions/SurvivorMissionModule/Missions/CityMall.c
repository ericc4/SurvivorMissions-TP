class CityMallMission extends SurvivorMissions {
    ...
    void SpawnSupplies() {
        if (m_MissionExtended) {
            ...
            for (int i = 0; i < MedSpawns.Count(); i++) {
                Object Med = GetGame().CreateObjectEx("TetracyclineAntibiotics", MissionBuilding.ModelToWorld("-5.836 -0.216 1.005"), ECE_PLACE_ON_SURFACE); //safe position
            ...
            MissionObject = ItemBase.Cast(GetGame().CreateObjectEx("MountainBag_Orange", RewardPos, ECE_PLACE_ON_SURFACE));
    ...
    void SpawnObjects() {
        ...
        if (m_MissionExtended) {
            ...
            EntityAI Cooker = EntityAI.Cast(GetGame().CreateObjectEx("PortableGasStove", MissionBuilding.ModelToWorld("-5.96 -0.2156 0.84"), ECE_PLACE_ON_SURFACE));
            ...
            m_MissionObjects.Insert(GetGame().CreateObjectEx("Paper", MissionBuilding.ModelToWorld("-9.74 -1.17 -7.49"), ECE_PLACE_ON_SURFACE));
            m_MissionObjects.Insert(GetGame().CreateObjectEx("Paper", MissionBuilding.ModelToWorld("-8.18 -1.17 -8.09"), ECE_PLACE_ON_SURFACE));
            m_MissionObjects.Insert(GetGame().CreateObjectEx("ShitGround", MissionBuilding.ModelToWorld("-9.54 -1.17 -7.69"), ECE_PLACE_ON_SURFACE));
            m_MissionObjects.Insert(GetGame().CreateObjectEx("ShitGround", MissionBuilding.ModelToWorld("-9.34 -1.17 -7.89"), ECE_PLACE_ON_SURFACE));
    ...
    override void ExtendMission() { //When player enters mission target zone at primary mission
        ...
        m_MissionMessage3 = "I have some rubles for you at the small police station. Be careful, there might be bandits around " + m_MissionSecondaryLoc + ". They could intercepted our little radio talk here. Good luck!";
