class GraveyardMission extends SurvivorMissions {
    ...
    void SpawnObjects() {
        ...
        stash = UndergroundStash.Cast(GetGame().CreateObjectEx("UndergroundStash", m_MissionPosition, ECE_PLACE_ON_SURFACE));
        stash.PlaceOnGround();
        ...
        if (ori[2] == -180 || ori[2] == 180) { //Bugfix for invisible Undergroundstashes
            ori[2] = 0;
        ...
        Object cc_stash = GetGame().CreateObjectEx("ClutterCutter2x2", MissionObject.GetPosition(), ECE_PLACE_ON_SURFACE);
