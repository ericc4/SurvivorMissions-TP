class UrbanMallMission extends SurvivorMissions { //UrbanMall is a copy of CityMall mission, it has equal functionality but uses different MissionBuilding
    ...
    void SpawnSupplies() {
        if (m_MissionExtended) {
            ...
            for (int i = 0; i < MedSpawns.Count(); i++) {
                Object Med = GetGame().CreateObjectEx("TetracyclineAntibiotics", MissionBuilding.ModelToWorld("-5.836 -0.216 1.005" - Offset), ECE_PLACE_ON_SURFACE); //safe position
            ...
            for (int j = 0; j < FoodSpawns.Count(); j++) {
                ...
                Object CanOfFood = GetGame().CreateObjectEx(Can, CanPos, ECE_PLACE_ON_SURFACE);
                CanOfFood.SetPosition(CanPos);
                m_MissionObjects.Insert(CanOfFood);
            }

            //Spawn extra weapon
            Object weapon = GetGame().CreateObjectEx("Mp133Shotgun", MissionBuilding.ModelToWorld("9.67 -1.16 0.53"), ECE_PLACE_ON_SURFACE);
            m_MissionObjects.Insert(weapon);
            weapon = GetGame().CreateObjectEx("Ammo_12gaPellets", MissionBuilding.ModelToWorld("9.57 -1.16 0.63"), ECE_PLACE_ON_SURFACE);
            weapon = GetGame().CreateObjectEx("Ammo_12gaPellets", MissionBuilding.ModelToWorld("9.51 -1.16 0.73"), ECE_PLACE_ON_SURFACE);
        } else {
            //Spawn orange MountainBag
            //vector pos = MissionBuilding.ModelToWorld( RewardsPosition );
            MissionObject = ItemBase.Cast(GetGame().CreateObjectEx("MountainBag_Orange", m_MissionPosition, ECE_PLACE_ON_SURFACE));
    ...
    void SpawnRewards() {
        //Spawn Lockpick
        GetGame().CreateObjectEx("Lockpick", m_MissionPosition, ECE_PLACE_ON_SURFACE);

        //new MissionObject after deleting orange bag
        MissionObject = ItemBase.Cast(GetGame().CreateObjectEx("MountainBag_Orange", RewardsPos2, ECE_PLACE_ON_SURFACE));
        ...
        for (int i = 0; i < PoliceInventory.Count(); i++) {
            ...
            Object item = GetGame().CreateObjectEx(InventoryType, InventoryPos, ECE_PLACE_ON_SURFACE);
    ...
    void SpawnObjects() {
        ...
        if (m_MissionExtended) { //Primary mission is selected
            ...
            for (int j = 0; j < Barricades.Count(); j++) {
                ...
                if (BarricadeType == "Land_Boat_Small1")
                    PlankBarricade = GetGame().CreateObjectEx(BarricadeType, BarricadePos, true, ECE_PLACE_ON_SURFACE);
                else {
                    PlankBarricade = GetGame().CreateObjectEx(BarricadeType, BarricadePos, ECE_PLACE_ON_SURFACE);
            ...
            EntityAI Cooker = EntityAI.Cast(GetGame().CreateObjectEx("PortableGasStove", MissionBuilding.ModelToWorld("-5.96 -0.2156 0.84" - Offset), ECE_PLACE_ON_SURFACE));
            ...
            m_MissionObjects.Insert(GetGame().CreateObjectEx("Paper", MissionBuilding.ModelToWorld("-9.74 -1.17 -7.49" - Offset), ECE_PLACE_ON_SURFACE));
            m_MissionObjects.Insert(GetGame().CreateObjectEx("Paper", MissionBuilding.ModelToWorld("-8.18 -1.17 -8.09" - Offset), ECE_PLACE_ON_SURFACE));
            m_MissionObjects.Insert(GetGame().CreateObjectEx("ShitGround", MissionBuilding.ModelToWorld("-9.54 -1.17 -7.69" - Offset), ECE_PLACE_ON_SURFACE));
            m_MissionObjects.Insert(GetGame().CreateObjectEx("ShitGround", MissionBuilding.ModelToWorld("-9.34 -1.17 -7.89" - Offset), ECE_PLACE_ON_SURFACE));

            Print("[SMM] Survivor Mission " + m_selectedMission + " :: " + m_MissionName + " ...primary mission deployed!");
        } else { //Secondary mission is selected
